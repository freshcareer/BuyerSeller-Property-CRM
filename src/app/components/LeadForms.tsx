...
import {
  ...
  Bed, Bath, Compass, Key, Tag, Car, Expand, Armchair, Layers, Clock, Upload, Image as ImageIcon, X
} from 'lucide-react';
...
export default function LeadForms({ options, defaultTab, hideTabs }: LeadFormsProps) {
  ...
  const cancelEdit = () => {
    setEditModeId(null);
    setFormData({ name: '', phone: '', email: '', propertyType: '', state: '', city: '', area: '', budgetOrPrice: '', customBudget: '',
    imageUrls: '', bedrooms: '', bathrooms: '', builtupArea: '', additionalSpaces: '', possessionStatus: '', facing: '', parking: '', description: '', tags: '', furnishing: '', balconies: '', propertyAge: '', notes: '', listingPurpose: activeTab === 'buy' ? 'buy' : 'sell' });
    setTouched({});
    setSubmitError(null);
  };
    ...
    try {
      if (activeTab === 'buy') {
        const { error: insertError } = await supabase.from('buyers_demand').insert({
          ...
          image_urls: formData.imageUrls ? formData.imageUrls.split(',').map(s => s.trim()).filter(Boolean) : [],
          status: 'new_lead',
