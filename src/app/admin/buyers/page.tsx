          ...
          <>
            ...
            <div className="block md:hidden divide-y divide-slate-100">
              ...
              {filteredBuyers.map((buyer) => (
                <div
                  key={buyer.id}
                  className={`p-4 space-y-3 transition-colors ${selectedIds.has(buyer.id) ? 'bg-blue-50/40' : 'bg-white'}`}
                >
                  {/* Card Header */}
                  <div className="flex items-start justify-between gap-2">
                    <div className="flex items-start gap-2 flex-1 min-w-0">
                      <button onClick={() => toggleSelect(buyer.id)} className="mt-0.5 shrink-0">
                        {selectedIds.has(buyer.id)
                          ? <CheckSquare className="w-4 h-4 text-blue-600" />
                          : <Square className="w-4 h-4 text-slate-400" />}
                      </button>
                      <div className="min-w-0">
                        <h4 className="font-bold text-slate-900 truncate">{buyer.name}</h4>
                        <a href={`tel:${buyer.phone}`} className="text-xs text-blue-600 font-medium flex items-center gap-1 mt-0.5">
                          <Phone className="w-3 h-3" />{buyer.phone}
                        </a>
                        {buyer.email && <p className="text-xs text-slate-400 mt-0.5 flex items-center gap-1"><Mail className="w-3 h-3" />{buyer.email}</p>}
                      </div>
                    </div>
                  ...
                  <div className="grid grid-cols-2 gap-2 text-xs">
                    <div className="flex items-center gap-1.5 bg-slate-50 rounded-lg px-2.5 py-2">
                      ...
                      <div className="min-w-0">
                        <p className="font-bold text-slate-700 capitalize truncate">{buyer.area.replace(/_/g, ' ')}</p>
                        <p className="text-slate-400">{buyer.city}</p>
                      </div>
                    </div>
                    <div className="flex items-center gap-1.5 bg-slate-50 rounded-lg px-2.5 py-2">
                      <Building className="w-3.5 h-3.5 text-slate-400 shrink-0" />
                      <p className="font-bold text-slate-700 capitalize truncate">{buyer.property_type.replace(/_/g, ' ')}</p>
                    </div>
                    <div className="flex items-center gap-1.5 bg-emerald-50 rounded-lg px-2.5 py-2 col-span-2">
                      <DollarSign className="w-3.5 h-3.5 text-emerald-600 shrink-0" />
                      <p className="font-bold text-emerald-700">{buyer.budget.replace(/_/g, ' ').replace('under ', '< ').replace('plus', '+')}</p>
                    </div>
                  </div>
                  ...
                  <div className="flex items-center gap-2 pt-1">
                    ...
                    <button
                      ...
                      className="flex-1 flex items-center justify-center gap-1.5 py-2 bg-blue-50 text-blue-700 hover:bg-blue-100 border border-blue-200 text-xs font-bold rounded-lg transition-all"
                    >
                      <GitCompare className="w-3.5 h-3.5" /> Match
                    </button>
                    <button
                      onClick={() => openEdit(buyer)}
                      className="flex-1 flex items-center justify-center gap-1.5 py-2 bg-slate-50 text-slate-700 hover:bg-slate-100 border border-slate-200 text-xs font-bold rounded-lg transition-all"
                    >
                    ...
                    {deleteConfirmId === buyer.id ? (
                      <div className="flex gap-1">
                        <button
                          ...
                          className="px-2.5 py-2 bg-rose-600 text-white text-xs font-bold rounded-lg flex items-center gap-1"
                        >
                          {deletingId === buyer.id ? <Loader2 className="w-3 h-3 animate-spin" /> : <Check className="w-3 h-3" />}
                        </button>
                        <button onClick={() => setDeleteConfirmId(null)} className="px-2 py-2 bg-white border border-slate-200 text-xs rounded-lg">
                          <X className="w-3 h-3" />
                        </button>
                      </div>
                    ) : (
                      <button
                        ...
                        className="px-3 py-2 bg-rose-50 text-rose-600 hover:bg-rose-100 border border-rose-200 text-xs font-bold rounded-lg transition-all"
                      >
                  ...
                  {filteredBuyers.map((buyer) => (
                    ...
                    >
                      ...
                      <td className="px-6 py-4 capitalize font-bold text-slate-700">
                        <div className="flex items-center gap-1.5"><MapPin className="w-4 h-4 text-slate-400" />{buyer.area.replace(/_/g, ' ')}</div>
                          ...
                          {deleteConfirmId === buyer.id ? (
                            <div className="flex items-center gap-1">
                              <button onClick={() => handleDelete(buyer.id)} disabled={deletingId === buyer.id} className="px-2.5 py-1.5 bg-rose-600 text-white text-xs font-bold rounded-lg flex items-center gap-1">
                                {deletingId === buyer.id ? <Loader2 className="w-3 h-3 animate-spin" /> : <Check className="w-3 h-3" />} Confirm
                              </button>
                              <button onClick={() => setDeleteConfirmId(null)} className="px-2 py-1.5 bg-white border border-slate-200 text-xs font-bold rounded-lg">
                                <X className="w-3 h-3" />
                              </button>
                            </div>
                          ) : (
